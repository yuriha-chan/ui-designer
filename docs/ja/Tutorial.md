...
![画面追加フォーム](screenshots/final/step2-02.png)

リストから「販売画面」を選択すると、左側の領域が「販売画面」の編集エリアになります。空白が表示されているはずです。

![画面一覧](screenshots/final/step2-03.png)
...
1. 右クリックして「コンテナ」を選択します。
2. 追加したコンテナの*中*を右クリックし、「テキスト」を選びます。
3. 「エンティティパス」というメニューが続けて表示されます。「エンティティパス」というのは、どのエンティティの、どのプロパティを表示するかを指定するものです。つまり、「このテキストコンポーネントは、どのデータを表示しますか？」と尋ねています。「本」を選び、展開されたリストから「タイトル」を選択します。
4. コンテナに「本 > タイトル」というコンポーネントが追加されました。
5. コンテナの中の何もない余白を再度右クリックし、「数値」を選びます。次に「本」→「価格」を選択します。
6. 最後に、コンテナに「ボタン」を追加し、ラベルとして「選択」を選びます。

![ボタンの画面選択](screenshots/final/step4-01.png)
...
![画面の切り替え](screenshots/final/step5-01.png)
...
![プロパティ名の編集](screenshots/final/step7-01.png)
